...
[[ "${lib_locale}" != "en_US" ]] && /usr/bin/defaults write "/Library/Preferences/.GlobalPreferences.plist" AppleLocale "en_US"
[[ "${root_locale}" != "en_US" ]] && /usr/bin/defaults write "/var/root/Library/Preferences/.GlobalPreferences.plist" AppleLocale "en_US"
...
get_time_stamp() {
    ...
    if printf '%s
' "$@" | grep --extended-regexp --line-regexp --quiet "\-f|\-\-format" && printf '%s
' "$@" | grep --extended-regexp --line-regexp --quiet "\-e|\-\-epoch" ; then
    ...
    [[ -z "${date_format}" && -z "${epoch}" ]] && date_format="${log_date_format}"
    [[ -n "${date_format}" ]] && args=( "'${date_format}'" "${args[@]}")
...
convert_date(){
    ...
    [[ -n "${date_string}" ]] && /bin/date -j -f "${input_format}" "$( fixup_date_string "${date_string}" )" +"${output_format}" 2>/dev/null
}
...
check_last_connection() {
    ...
    if [[ $( convert_date "${fixed_date}" "${falconctl_date_format}" "%s" ) -lt $( get_time_stamp --adjust -v-"${2}"d --epoch ) ]]; then
...
if [[ "${connectionState}" == "connected" ]]; then
    ...
    if [[ -n "${established}" ]] && [[ -n "${lastEstablished}" ]]; then
        ...
        if [[ $( convert_date "${established}" "${falconctl_date_format}" "%s" ) -ge $( convert_date "${lastEstablished}" "${falconctl_date_format}" "%s" ) ]]; then
